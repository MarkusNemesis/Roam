...
pushd build\exe.win-amd64-3.11
ECHO %CD%
...
SET NAME=%2
CALL %BASE%scripts\installer\makesfx.bat "%BASE%release\Roam%NAME% Installer " build\exe.win-amd64-3.11
CALL %BASE%scripts\installer\makesfx.bat "%BASE%release\Roam%NAME% Installer - Silent" build\exe.win-amd64-3.11 -s
GOTO END
